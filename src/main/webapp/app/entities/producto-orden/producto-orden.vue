<template>
  <div>
    <h2 id="page-heading" data-cy="ProductoOrdenHeading">
      <span v-text="$t('pedidosrapiadminApp.productoOrden.home.title')" id="producto-orden-heading">Producto Ordens</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('pedidosrapiadminApp.productoOrden.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ProductoOrdenCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('pedidosrapiadminApp.productoOrden.home.createLabel')"> Create a new Producto Orden </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && productoOrdens && productoOrdens.length === 0">
      <span v-text="$t('pedidosrapiadminApp.productoOrden.home.notFound')">No productoOrdens found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="productoOrdens">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('pedidosrapiadminApp.productoOrden.cantidad')">Cantidad</span></th>
            <th scope="row"><span v-text="$t('pedidosrapiadminApp.productoOrden.precioTotal')">Precio Total</span></th>
            <th scope="row"><span v-text="$t('pedidosrapiadminApp.productoOrden.producto')">Producto</span></th>
            <th scope="row"><span v-text="$t('pedidosrapiadminApp.productoOrden.cart')">Cart</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="pedidosrapiadminApp.productoOrden.delete.question"
          data-cy="productoOrdenDeleteDialogHeading"
          v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-productoOrden-heading" v-text="$t('pedidosrapiadminApp.productoOrden.delete.question', { id: removeId })">
